...
LPDIRECT3DTEXTURE9 TextureManager::LoadTextureFromFile(char *fName,char *path)
{
	//生成用オブジェクト
  ...
  char c_f_name[200] = "";
  //stringからchar*に形式変換
	strcpy(c_f_name,s_f_name.c_str());

	//重複ロード防止のためファイル名チェックを行う
	if((create=this->CheckFileName(c_f_name))!=NULL)
	{
		//登録済みオブジェクトを戻り値とする
		return create->lpTex;
	}
	...
	if(FAILED(this->LoadTextureFromFile(create,c_f_name,path)))
	{
...
DxTexture* TextureManager::LoadTextureFromFileEx(char *fName,char *path)
{
	//生成用オブジェクト
	DxTexture *create=NULL;
	...
	create = new DxTexture;
	//生成用オブジェクトにパス及びファイル名からロード
	if(FAILED(this->LoadTextureFromFileEx(create,c_f_name,path)))
	{
		//ロードに失敗した場合、領域確保した情報を破棄
		SAFE_DELETE(create);
		return NULL;
...
//**************************************************************************
//イメージファイルからDxTextureをロード　　（簡易版）
HRESULT TextureManager::LoadTextureFromFile(DxTexture* tex,char *fName,char *path)
{
	HRESULT hr=S_OK;
	char str[256]="";
	...
	D3DXGetImageInfoFromFile(str,&tex->info);

	if(FAILED(D3DXCreateTextureFromFile(this->dev,str,&tex->lpTex)))
	{
		return E_FAIL;
	}

	//ファイル名の領域確保
	tex->file_name = new char[strlen(fName)+1];
	//ファイル名の設置
	strcpy(tex->file_name,fName);

    return S_OK;
}
//イメージファイルからDxTextureをロード　　（詳細版）
HRESULT TextureManager::LoadTextureFromFileEx(DxTexture* tex,char *fName,char *path)
{
	HRESULT hr=S_OK;
	char str[256]="";
...
{
	//ファイル名の破棄
	SAFE_DELETE_ARRAY(tex->file_name);
	//テクスチャの開放
...
DxTexture* TextureManager::CheckFileName(char *fName)
{
	char str1[25]="";
	char str2[25]="";
	...
	{
		strcpy(str2,this->tex_data[i]->file_name);
		if(!strcmp(str1,str2))
		{
